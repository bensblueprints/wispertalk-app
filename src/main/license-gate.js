 ...
 * Vendored next to whop-license.js (OneTimeSuite canonical pattern).
 *
 * Usage in main.js:
  ...
  for (;;) {
    try {
      ...
      if (r.firstActivation) {
        dialog.showMessageBoxSync({ type: 'info', title: 'Activated', message: 'WisperTalk is activated on this device. Enjoy!' });
      }
      return true;
    } catch (e) {
      if (e.code === 'DEVICE_LIMIT' && Array.isArray(e.devices) && e.devices.length) {
        ...
        const { response } = await dialog.showMessageBox({
          type: 'error', title: 'Purchase required', buttons: ['Sign in with Whop', 'Buy WisperTalk', 'Quit'], cancelId: 2,
          message: e.message || 'Could not verify your WisperTalk purchase.',
          detail: 'WisperTalk unlocks with the Whop account that purchased it — no license key needed.',
        });
        if (response === 0) continue;
        if (response === 1) { shell.openExternal(cfg.buyUrl); continue; }
      }
      app.quit();
      return false;
    }
...
/* Renderer-side license IPC for the Settings → License pane. */
